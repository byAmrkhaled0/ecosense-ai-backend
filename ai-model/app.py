...
def clean_response_text(value):
  """
  Recursively remove leading/trailing spaces from all strings in API responses.
  This keeps responses clean after removing emojis and also keeps Arabic readable.
  """
  if isinstance(value, dict):
    return {key: clean_response_text(val) for key, val in value.items()}
  if isinstance(value, list):
    return [clean_response_text(item) for item in value]
...
if os.path.exists(RECOMMENDATIONS_FILE):
  ...
  solutions = {
    ...
    "Moderate Stress": "يوجد إجهاد متوسط على النبات. راجع الري ودرجة الحرارة والإضاءة وقم بضبطهم لتحسين الحالة.",
...
def build_recommendations(data, status, image_result=None, safety_flags=None):
  ...
  if status == "Healthy":
    ...
    recs.append(f"النبات ({crop_type}) يعاني من إجهاد متوسط ويحتاج متابعة.")
  else:
  ...
  if temperature < 20:
    recs.append("درجة الحرارة منخفضة؛ يُفضل رفع الحرارة أو تقليل التعرض للبرودة.")
  elif 20 <= temperature <= 35:
    recs.append("درجة الحرارة مناسبة.")
  else:
    recs.append("درجة الحرارة مرتفعة؛ حسّن التهوية أو التبريد.")
  ...
  if humidity < 40:
    recs.append("الرطوبة الجوية منخفضة؛ قد يزيد ذلك من الإجهاد المائي.")
  elif humidity <= 80:
    recs.append("الرطوبة الجوية مناسبة.")
  else:
    recs.append("الرطوبة الجوية مرتفعة؛ حسّن التهوية لتقليل فرص الأمراض.")
  ...
  if light == "Low":
    recs.append("الإضاءة ضعيفة؛ زوّد الضوء أو مدة التعرض.")
  elif light == "Medium":
    recs.append("الإضاءة متوسطة؛ استمر في المتابعة.")
  elif light == "Sufficient":
    recs.append("الإضاءة مناسبة للنمو.")
  else:
    recs.append("قيمة الإضاءة غير معروفة؛ تأكد من إرسال قيمة صحيحة.")

  if image_result:
    for item in image_result.get("image_recommendations", []):
      if item not in recs:
        recs.append(str(item).strip())
...
def apply_safety_layer(sensor_status, risk_factors, image_result=None):
  ...
  if image_result:
    ...
    # Moderate visual disease should remain Moderate Stress.
    # Do not upgrade normal Leaf Spot suspicion to High Stress unless the image itself
    # indicates severe disease or the visual damage is clearly high.
    if visual_problem == "Severe Leaf Spot / Fungal Suspicion":
      final_status = choose_worst_status(final_status, "High Stress")
      safety_flags.append("SEVERE_IMAGE_DISEASE_OVERRIDE")

    elif visual_problem == "Leaf Spot / Fungal Suspicion":
      final_status = choose_worst_status(final_status, "Moderate Stress")

    # Only severe visual evidence upgrades the final decision to High Stress.
    if damaged_ratio >= 0.60 or dark_spot_ratio >= 0.30:
      final_status = choose_worst_status(final_status, "High Stress")
...
def _status_confidence(confidence_dict, status):
  """Return confidence value for a given status from the sensor probability dict."""
  if not isinstance(confidence_dict, dict):
    return None
  value = confidence_dict.get(status)
  if value is None:
    return None
  return round(float(value), 3)


def _final_confidence(final_status, sensor_status, sensor_confidence, image_status=None, image_confidence=None):
  """Pick the confidence that belongs to the source driving final_status."""
  image_confidence = None if image_confidence is None else round(float(image_confidence), 3)

  if image_status in STATUS_PRIORITY and final_status == image_status and image_confidence is not None:
    return image_confidence

  sensor_value = _status_confidence(sensor_confidence, sensor_status)
  if final_status == sensor_status and sensor_value is not None:
    return sensor_value

  # If the final decision was raised because of image evidence, use the image confidence.
  # This avoids returning Healthy sensor confidence for an image-driven Moderate/High result.
  if image_status in STATUS_PRIORITY and final_status != sensor_status and image_confidence is not None:
    return image_confidence

  # Safety override from sensor readings: no direct rule confidence exists, so return
  # the strongest available sensor evidence.
  candidates = []
  if sensor_value is not None:
    candidates.append(sensor_value)
  if isinstance(sensor_confidence, dict) and sensor_confidence:
    candidates.append(max(float(v) for v in sensor_confidence.values()))
  if image_confidence is not None:
    candidates.append(image_confidence)
...
def build_image_only_response(image_result, crop_type="Unknown"):
  ...
  raw_confidence = image_result.get("confidence")
  if raw_confidence is not None:
    confidence = round(float(raw_confidence), 3)
  elif image_stress == "Healthy":
  ...
  if image_stress == "Healthy":
    status = "Healthy"
    disease_name = image_result.get("disease_name", "No Clear Disease Detected")
  else:
    ...
    disease_name = image_result.get("disease_name", visual_problem)
  ...
  return clean_response_text({
    ...
    "disease_name_ar": image_result.get("disease_name_ar", visual_problem_ar),
    "confidence": confidence,
    "image_confidence": confidence,
    "final_confidence": confidence,
    "image_status": image_stress,
...
def predict_sensors():
  ...
  try:
    ...
    return jsonify(clean_response_text(response_data)), 200
...
def get_history():
  try:
    history = load_history()
    return jsonify(clean_response_text({
      "count": len(history),
      "history": history
    })), 200
  except Exception as e:
